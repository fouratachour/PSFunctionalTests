...
describe('Verify sending an email to reset the password', function () {
    ...
    describe('check sending an email of rest the password', function (done) {
        it('should check the received email ', function (done) {
            global.fctname = this.test.title;
            this.client
                .waitForExist(this.selector.EmailConfigPage.forgotten_password_button, 90000)
                .click(this.selector.EmailConfigPage.forgotten_password_button)
                .waitForExist(this.selector.EmailConfigPage.forgotten_password_input, 90000)
                .setValue(this.selector.EmailConfigPage.forgotten_password_input, 'pub@prestashop.com')
                .click(this.selector.EmailConfigPage.forgotten_password_email_button)
                .call(done);
        });
    });
